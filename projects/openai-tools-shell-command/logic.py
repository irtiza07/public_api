...
def handle_user_query(user_query: str, previous_response_id: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if shell_calls:
            ...
            for shell_call in shell_calls:
                ...
                timeout_ms = action.get('timeout_ms', None)
                max_output_length = action.get('max_output_length', None)
                
                print(f"\n   🔨 Shell Call ID: {call_id}")
                print(f"   📝 Commands: {commands}")
                print(f"   ⏱️  Timeout: {timeout_ms}ms" if timeout_ms else "   ⏱️  Timeout: default")
